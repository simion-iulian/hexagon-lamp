...
function loadState() {
  ...
  if (switchState == 1) {
    ...
    for (var i = 0; i < strip.length; i++) strip.setPixel(i, 0x000000);
  }
  strip.render();
}
...
let switchCharacteristic = new SwitchCharacteristic(
  switchSettings.service_id,
  "Switch"
);
let patternCharacteristic = new PatternCharacteristic(
  patternSettings.service_id,
  "Pattern"
);
let colorCharacteristic = new ColorCharacteristic(
  colorSettings.service_id,
  "Color (24-bit)"
);
...
bleno.on("advertisingStart", function (error) {
  ...
  if (error) {
    ...
    console.log("started...");
    //console.log(bleno);
    bleno.setServices([neopixelService]);
  }
});
